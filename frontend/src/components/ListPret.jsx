...
function ListPret({ prets, onDelete, onEdit }) {
 ...
 const pretsFiltres = prets.filter(pret =>
  ...
  return (
    <div className="list-pret-container">
      <h2><ClipboardList className="section-icon" aria-hidden="true" /> Liste des Prêts</h2>
<div className="search-container">
 ...
 {pretsFiltres.length === 0 ? (
  ...
  <div className="empty-message">
    {searchTerm ? (
      <>
        <p><Search className="empty-icon" aria-hidden="true" /> Aucun résultat pour "<strong>{searchTerm}</strong>"</p>
        <p>Essayez une autre recherche</p>
      </>
    ) : (
      <>
        <p>Aucun prêt enregistré.</p>
        <p>Allez à l'onglet <FilePlus className="inline-icon" aria-hidden="true" /> "Ajouter un prêt" pour commencer.</p>
      </>
    )}
  </div>
      ) : (
            ...
            {pretsFiltres.map(pret => (
                <tr key={pret.id}>
                  ...
                  <td className="montant-a-payer">
                    {calculateMontantAPayer(pret.montant, pret.tauxPret).toFixed(2)}
                  </td>
                 <td className="actions">
  <button className="btn-edit" onClick={() => onEdit(pret.id)} aria-label="Modifier le prêt"><Pencil aria-hidden="true" /></button>
  <button className="btn-delete" onClick={() => onDelete(pret.id)} aria-label="Supprimer le prêt"><Trash2 aria-hidden="true" /></button>
</td>
                </tr>
              ))}
            </tbody>
